...
0. We first initialize our neural network's parameters $\theta_0$

1. Then we draw random points $$s_n = \{(t_n, x_n), (\tau_n, z_n), w_n\}$$ from our domain. Here $(t_n, x_n) \in [0, T] \times \Omega$, $(\tau_n, z_n) \in [0, T] \times \partial \Omega$ and $w_n \in \Omega$. The random sampling is performed according to the respective probability densities $\nu_1, \nu_2$ and $ \nu_3$.
...
Of course, the upper mentioned notes regarding SGD apply also here.  In particular, one should take into account to adapt the learning rate and the fact that the algoritm might only converge to a local minimum.
