...
$$G_1(\theta_n, s_n) = (\frac{\partial f}{\partial t}(t_n, x_n; \theta_n) + L_1f(t_n, x_n; \theta_n) + \frac{1}{2} \sum^d_{i,j = 1} \rho_{i,j} \sigma_i(x_n) \sigma_j(x_n) \frac{\partial^2 f}{\partial x_i x_j}(t_n, x_n; \theta) )^{2}$$

Applying the chain rule we can calculate the derivative of $G_1(\theta_n, s_n)$ by

$$\nabla_{\theta} G_1(\theta_n, s_n) = 2(\frac{\partial f}{\partial t}(t_n, x_n; \theta_n) + L_1f(t_n, x_n; \theta_n) + \frac{1}{2} \sum^d_{i,j = 1} \rho_{i,j} \sigma_i(x_n) \sigma_j(x_n) \frac{\partial^2 f}{\partial x_i x_j}(t_n, x_n; \theta) ) + \\ \nabla_{\theta} (\frac{\partial f}{\partial t}(t_n, x_n; \theta_n) + L_1f(t_n, x_n; \theta_n) + \frac{1}{2} \sum^d_{i,j = 1} \rho_{i,j} \sigma_i(x_n) \sigma_j(x_n) \frac{\partial^2 f}{\partial x_i x_j}(t_n, x_n; \theta) $$


With this scheme we can increase the algorithms speed, however, we introduce a bias of $\mathcal{O}(\sqrt{\Delta})$ into the calculation of $\nabla_{\theta}G_1(\theta_n, s_n)$.
