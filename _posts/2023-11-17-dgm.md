...
### The reference PDE
In the paper the DGM is applied to the following parabolic PDE. However, the algorithm can easily be modified to other classes of PDEs.

$$ \frac{\partial u}{\partial t}(t, x) + Lu(t,x) = 0, (t,x) \in \[0, T\] \prod \Omega \\ u(0, x) = u_0(x) \\ u(t, x) = g(t, x)$$
